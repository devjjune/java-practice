...
## 🎨 객체를 쪼개기를 남발하지 말자 : 무분별한 getter의 생성
이전에는 객체를 필요 이상으로 잘게 쪼갰고, 그만큼 getter도 많이 생겼다.   
그러나 getter가 많아지면 캡슐화가 파괴되어 객체의 의미가 없고, 클래스 간 의존도가 높아지며, 코드가 절차지향적으로 변한다.  
(Tell, Don't Ask 원칙에 위배)  
→ **getter 사용의 기준: 비교나 계산 등 로직을 위한 getter X , 출력을 위한 getter O**
