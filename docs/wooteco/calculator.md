...
### 💥 입력 로직 에러 - 입력값의 개행 문자(\n)를 리터럴로 착각 
요구사항에서 커스텀 구분자 선언부에 개행 문자 `\n`이 등장하는데, 이를 문자열 리터럴 텍스트로 잘못 이해했다.

콘솔 입력에서는 엔터가 곧 입력 종료이기 때문에, 개행을 문자열 내부에 포함시키기 위해 엔터 전후의 입력값을 합치도록 입력 로직을 수정했다.   
다만 이 과정에서 항상 두 줄 입력을 강제하는 문제가 생겨, 최종적으로는 `//`로 시작하는 커스텀 구분자 입력일 때만 두 줄 입력을 받도록 개선했다.
...
### 🧩 테스트 코드 작성법
[🌟 테스트 코드 작성법](https://velog.io/@hjy648012/%ED%85%8C%EC%8A%A4%ED%8A%B8-%EC%BD%94%EB%93%9C-%EC%9E%91%EC%84%B1%EB%B2%95)
